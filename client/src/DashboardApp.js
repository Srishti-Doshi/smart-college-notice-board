export { default } from './DashboardAppV2';
/*
...
function DashboardApp() {
  ...
  const isSnoozed = Boolean(snoozeUntil && snoozeUntil > Date.now());
  ...
  const notificationStatus = useMemo(() => {
    ...
    if (notificationPermission === 'granted' && !isSnoozed) {
      return 'live';
    }

    return 'paused';
  }, [isSnoozed, notificationPermission]);
  ...
  }, [isSnoozed, notificationPermission, subscribedCategories]);

  useEffect(() => {
    if (digestTimerRef.current) {
      window.clearInterval(digestTimerRef.current);
    }
    ...
    digestTimerRef.current = window.setInterval(() => {
      if (
        notificationPermission !== 'granted' ||
        isSnoozed ||
        pendingDigestNotices.length === 0
      ) {
        return;
      }

      const titles = pendingDigestNotices.slice(0, 3).map((notice) => notice.title);
      new Notification(`Notice digest (${pendingDigestNotices.length})`, {
        body: titles.join(' • '),
      });
      recordAlertSent();
      setPendingDigestNotices([]);
    }, 120000);
  ...
  }, [isSnoozed, notificationMode, notificationPermission, pendingDigestNotices]);
  ...
  const handleSnoozeOneHour = () => {
    const snoozeTarget = Date.now() + 60 * 60 * 1000;
    setSnoozeUntil(snoozeTarget);
    setSubmitMessage('Alerts muted for 1 hour.');
  };

  const handleSnoozeUntilTomorrow = () => {
    const tomorrowMorning = new Date();
    tomorrowMorning.setDate(tomorrowMorning.getDate() + 1);
    tomorrowMorning.setHours(8, 0, 0, 0);
    setSnoozeUntil(tomorrowMorning.getTime());
    setSubmitMessage('Alerts muted until tomorrow morning.');
  };

  const handleResumeAlerts = () => {
    setSnoozeUntil(null);
    setSubmitMessage('Alerts resumed.');
  };

  const handleSendTestNotification = () => {
    if (notificationPermission !== 'granted') {
      setSubmitMessage('Enable notifications first to send a test alert.');
      return;
    }

    new Notification('SmartNotice Test Alert', {
        ...
        <div className="max-w-6xl mx-auto px-6 py-5 md:px-8 flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm text-slate-300">
              A focused campus announcement hub with urgent priorities, searchable history,
              and category-based alerting.
            </p>
          </div>
            ...
            {mode === 'student' && feedView === 'active' && pinnedNotices.length > 0 && (
              <section
                ...
                className={`mb-10 rounded-[30px] transition ${
                  highlightPinned
                    ? 'bg-amber-50/70 p-4 ring-2 ring-amber-300'
                    : ''
                }`}
              >
              ...
              {notices.length > 0 ? (
                ...
                <div className="rounded-[30px] border border-dashed border-slate-300 bg-white/80 px-8 py-16 text-center shadow-sm">
                  ...
                  <p className="mt-3 max-w-xl mx-auto text-sm text-slate-500">
                    Try resetting the filters, switching feeds, or creating a new notice from
                    the admin panel.
                  </p>
                </div>
              )}
        ...
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-slate-950/55 px-6">
          <div className="w-full max-w-md rounded-[30px] border border-slate-200 bg-white p-8 shadow-[0_20px_55px_rgba(15,23,42,0.18)]">
            ...
            <p className="mt-2 text-sm text-slate-500">
              Login as Admin or HOD to manage notices.
            </p>
